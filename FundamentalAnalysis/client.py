...
from .request import FundamentalRequest
from .response import FundamentalResponse

logger = get_logger(__name__)

class FundamentalWebSocketClient:
    ...
    async def fetch_fundamentals(self, symbol: str, from_date: str, to_date: str) -> FundamentalResponse:
        if not self.connection:
            raise ConnectionError("Not connected to WebSocket. Call connect() first.")
        
        request = FundamentalRequest(symbol, from_date, to_date)
        request_data = json.dumps(request.to_dict())
        
        try:
            logger.info(f"Sending request for symbol {symbol}")
        ...
        try:
            ...
            return FundamentalResponse(raw_response)
        except Exception as e:
