...
class MidasRegression:
    ...
    def fit(self, daily_returns: pd.Series, sparse_quarterly: pd.Series) -> None:
        ...
        best_mse = float("inf")
        best_params = (0.0, 0.0, 0.1, 60)

        lag_grid = [30, 45, 60, 90]
        theta_grid = np.linspace(0.001, 0.5, 30)
        ...
        for lag in lag_grid:
            for theta in theta_grid:
                ...
                for d in report_dates:
                    ...
                    weights = np.exp(-theta * np.arange(1, lag + 1))
                    weights /= np.sum(weights)
                    lagged_returns = returns_np[t_idx - lag : t_idx]
                ...
                try:
                    ...
                    if mse < best_mse:
                        best_mse = mse
                        best_params = (beta[0], beta[1], theta, lag)
                except Exception:
                    continue

        self.beta0, self.beta1, self.theta, self.lag_length = best_params
